...
u8 Mapper52::write(u16 addr, u8 v)
{
    if (addr < 0x8000)
    {
        // check if cartridge switch register has been written.
        // If not, this write ends up there, otherwise in RAM
...
u8 Mapper52::chr_write(u16 addr, u8 v)
{
    return chr[addr] = v;
}

void Mapper52::signal_scanline()
{
    if (irqCounter == 0)
